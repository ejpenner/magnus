            ...
            <div class="item-details">
                <h5><strong><a href="{{ action('OpusController@show', [$opus->id]) }}">{{ $opus->title }}</a></strong>
                    @if(!isset($showName) or $showName)
                        <br><a href="{{ action('ProfileController@show', $opus->uslug) }}">{!! Helpers::usernameID($opus->user_id) !!}</a>
                    @endif
                </h5>
            </div>
        </div>
        <div class="gallery-operations">
            @if(Auth::check() and (Helpers::isOwner(Auth::user(), $opus) or Auth::user()->atLeastHasRole(config('roles.globalModerator'))))
